...
const SearchList = ({ filteredStocks, userStocks, handleAddStock, handleRemoveStock }) => (
...
const SearchListItem = ({ stock, userStocks, handleAddStock, handleRemoveStock }) => {
    ...
    return (
        <AddStocklist><Stocksymbol>{stock.symbol}</Stocksymbol>
            <Stockdescription>{stock.description}</Stockdescription>
            {!(userStocks.some(stateStock => stateStock.symbol === stock.symbol)) ?
                <AddDeleteButton onClick={(e) => handleAddStock(stock)}><AddDeleteText>+</AddDeleteText></AddDeleteButton> :
                <AddDeleteButton primary onClick={(e) => handleRemoveStock(stock)}><AddDeleteText>-</AddDeleteText></AddDeleteButton>}
        </AddStocklist>);
}
